...
impl<'a> Layer<'a> for TrunkLayer<'a> {
    fn generate(
        ...
        let split_rate = |n: usize| split * (-(n as f64 - SPLIT_FALLOFF_PEAK).powi(2)).exp();
        let branch_rate =
            |n: usize| 3.0 * branch * (-(n as f64 - 0.0).powi(2)).div_euclid(100.0).exp();
                ...
                .flat_map(|&j| {
                    ...
                    if is_split && j == split_index {
                        let normal = Normal::new(
                            spread * ANGLE_SPREAD_POSITIVE.to_radians() + LEAN_BIAS.to_radians(),
                        ...
                        let normal = Normal::new(
                            spread * ANGLE_SPREAD_NEGATIVE.to_radians() + LEAN_BIAS.to_radians(),
                            variability * VARIABILITY_MODIFIER,
                        )
                        .unwrap();
                        let angle_b = normal
                        ...
                        let normal = Normal::new(
                            ...
                            spread * variability * VARIABILITY_MODIFIER,
                        )
                        .unwrap();
